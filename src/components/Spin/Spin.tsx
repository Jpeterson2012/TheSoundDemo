...
const color2 = "#" + ((1 << 24) * Math.random() | 0).toString(16).padStart(6, "0");
const color3 = "#" + ((1 << 24) * Math.random() | 0).toString(16).padStart(6, "0");
...
const Spin = (options: Record<string, any>) => {
    
    const albumImage = (() => {
        if (options.custom) return <div className='svgImage'>{options.custom}</div>
        else return <img className='svgImage' src={options.url}/>
    });
    ...
    return (
        <div className='svgContainer'>
            ...
            <svg id='svg1' viewBox="0 0 400 400">
                <g id="record"  style={!options.active ? {animationPlayState: 'paused'} :  options.paused ? {animationPlayState: 'paused'} : ({animationPlayState: 'running'})}>
